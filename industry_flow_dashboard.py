...
function fillTheme(id, snapshot, frame) {
  ...
  themeCard.innerHTML = top
    ? `<span class="theme-line">Most names in this ${escapeHTML(label)} window sit in <strong>${escapeHTML(top[0])}</strong> (${top[1]} of them). The table below is the full window, not that industry only.</span>`
